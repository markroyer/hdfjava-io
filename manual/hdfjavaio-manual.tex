...
Octave \cite{octave-url}.  This document discusses \hdfc\ a library
that allows Java programs to read and write Octave Hierarchical Data
Format 5 (HDF5) \cite{hdf5-url}.  This specification is based off of
the Matlab 7.3 format (Need reference here).
...
that is in Octave.  In an attempt to make it clear which data form is
being discussed, the following convention will be used.

\noindent
Java types will appear as

\noindent
\java{boolean}

\noindent
Octave types will appear as

\noindent
\octave{logical}

\noindent
And finally, HDF5 file types will be represented using 

\noindent
\hdf{64-bit floating-point}

Generally, the notation given above will be used throughout the
document.  However, code examples will use formatting and markup
common for the language that it is written in.
...
%%  LocalWords:  writeObjectToFile checkedList qf
